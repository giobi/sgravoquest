...
/**
 * Movimento a griglia tile-locked, stile Pokémon GBA.
 * Un tasto = uno scatto di un tile, con tween fra le celle.
...
export class GridPlayer {
  ...
  update(): void {
    ...
    if (!dir) {
      ...
      this.sprite.setFrame(`misa-${this.facing === "up" ? "back" : this.facing === "down" ? "front" : this.facing}`);
      return;
    }
    ...
    if (this.isBlocked(ntx, nty)) {
      this.sprite.setFrame(`misa-${dir === "up" ? "back" : dir === "down" ? "front" : dir}`);
      return;
    }
  ...
  static createAnims(scene: Phaser.Scene): void {
    const mk = (dir: Dir, prefix: string) => {
      ...
      scene.anims.create({
        key: `walk-${dir}`,
        frames: scene.anims.generateFrameNames("misa", {
          prefix: `misa-${prefix}-walk.`,
          start: 0,
          end: 3,
          zeroPad: 3,
        }),
