...
const SAND_ROW = 5; // riga spiaggia
...
export class LakefrontScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const blocked = (tx: number, ty: number): boolean => {
      ...
      return this.grid[ty][tx] === WATER; // non entri nel lago
    };
    ...
    this.add.text(W / 2, 4, "Lungolago di Pallanza", {
      fontFamily: "monospace", fontSize: "12px", color: "#ffffff", backgroundColor: "#00000088",
  ...
  private onTile(_tx: number, ty: number): void {
    if (this.busy) return;
    // incontro avvicinandosi all'acqua (riga spiaggia)
